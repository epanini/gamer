   ...
   if ( FirstTime )
   {
      if ( MPI_Rank == 0 )
      {
         ...
         for (int c=0; c<Merger_Coll_NumHalos; c++){
            ...
            fprintf( File_User, " %13s%1d %13s%1d %13s%1d", "Inflow",        c, "Inflow_cold",   c, "ColdGasMass",   c );
            fprintf( File_User, " %13s%1d %13s%1d %13s%1d", "Mdot",          c, "Pdot(cgs)",     c, "Edot(cgs)",     c );
            ...
            fprintf( File_User, " %13s%1d %13s%1d %13s%1d", "mass_BH[Msun]", c, "Mdot",          c, "NVoidCell",     c );
            ...
            fprintf( File_User, " %13s%1d %13s%1d %13s%1d", "num_par_sum",   c, "GasMass",       c, "ParMass",       c );
         }
         fprintf( File_User, "\n" );
         fclose( File_User );
      }
...
// sum over the variables and convert units 
   ...
   if ( MPI_Rank == 0 )
   {
      ...
      for (int c=0; c<Merger_Coll_NumHalos; c++){
         ...
         fprintf( File_User, " %14.7e %14.7e %14.7e", Inflow[c]* (UNIT_M / UNIT_T) * (Const_yr/Const_Msun), Inflow_cold[c]*(UNIT_M / UNIT_T) * (Const_yr/Const_Msun), ColdGasMass[c]*(UNIT_M/Const_Msun) );
         fprintf( File_User, " %14.7e %14.7e %14.7e", Mdot[c]*(UNIT_M / UNIT_T) * (Const_yr/Const_Msun), Pdot[c]*UNIT_M*UNIT_V/UNIT_T, Edot[c]*UNIT_E/UNIT_T );
         ...
         fprintf( File_User, " %14.7e %14.7e %14d",   Bondi_MassBH[c]*UNIT_M/Const_Msun, Mdot_BH[c]*(UNIT_M / UNIT_T) * (Const_yr/Const_Msun), SinkNCell_Sum[c] );
         ...
         fprintf( File_User, " %14d   %14.7e %14.7e", num_par_sum[c], GasMass[c]*(UNIT_M/Const_Msun), ParMass[c]*(UNIT_M/Const_Msun)  );
